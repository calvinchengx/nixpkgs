...
let
  ...
  # TODO: generate multiple config files for multiple instances
  configFile = cfg: ''
  '';
...

{
  ...
  config = mkIf config.services.rethinkdb.enable {
    ...
    systemd.services.rethinkdb = {
      ...
      preStart = ''
        if ! test -e ${cfg.dbpath}; then
            install -d -m0644 -o ${cfg.user} -g ${cfg.group} ${cfg.dbpath}
    ...
    users.extraUsers.mongodb = mkIf (cfg.user == "rethinkdb")
      { name = "rethinkdb";
